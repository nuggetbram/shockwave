...
//hgb
uint256 hashGenesisBlock("0x000000000000000000000000000000001cc1f6befa269dc717f7d8f876e5639d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // TorvaCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "TorvaCoin Signed Message:\n";
...
{
    // TorvaCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
double static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    float block = static_cast<double>(nHeight);

    double nSubsidy = (-35.0 / (8.0 * sqrt(219.0))) * sqrt(block) + 350.0;
...
static const int64 nTargetTimespan = 24 * 60 * 60; // TorvaCoin: 1 day
static const int64 nTargetSpacing = 1.5 * 60; // TorvaCoin: 1 minute
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // TorvaCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
        static const int64 BlocksTargetSpacing = 1.5 * 60; // TorvaCoin: 1 minute
        static const unsigned int TimeDaySeconds = 24 * 60 * 60; // TorvaCoin: 1 day
...
{
    // TorvaCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    // vMerkleTree: b0019d92bc054f7418960c91e252e7d24c77719c7a30128c5f6a827c73095d2a 
        ...
        const char* pszTimestamp = "MH17 Is Shot Down Over Ukraine, Russian Sepratists are blamed. Putin is mad...";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1405830000;
        block.nBits    = 0x1e0ffff0;
		block.nNonce   = 0;
		//bnnn
        if (fTestNet)
        {
            block.nTime    = 1405811400;
            block.nNonce   = 0;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xb3b2819e6188b700cafd10b7a9127d61ce5005ac7287333aa6d6c1ddb8372f04"));
        //merklerootentry
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // TorvaCoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TorvaCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TorvaCoinMiner : ProcessBlock, block not accepted");
    }
